...
# Reading the dataset
ipl = pd.read_csv('feature_engineering6.csv')

# Data Cleaning by removing the columns not necessary for this model

ipl = ipl.drop([ 'ball','match_id', 'batting_team', 'bowling_team', 'winner', 'city'], axis=1)
...
ip_dataset = ipl[['innings', 'batteam',
                  'bowlteam',
                    'over', 'runs', 'runs_tillnow', 'wickets', 'last_5_over_wickets',
                  'last_5_over_runs']]
...
def testing(over, ball_in_the_over, batting_team, bowling_team, runs_in_last_5_overs, wickets_in_last_5_overs,
    ...
    if over < 5 and runs_in_last_5_overs !=current_score:
        return "As the overs are less than 5 , The current score should be same as score in last 5 overs "
    ...
    if bowling_team == batting_team:
        ...
        if new_prediction<current_score:
            return int(current_score+runs_in_last_5_overs)
        else:
